...
# uncomment these lines + the two lines at the end to start and kill calibre-server from this script if on linux:
#calibre-server --max-cover=300x400 --daemonize --pidfile=/tmp/calibre.pid h-p 8080
# or this one + the two lines at the end if on osx:
#/Applications/calibre.app/Contents/MacOS/calibre-server --max-cover=300x400 --daemonize --pidfile=/tmp/calibre.pid h-p 8080
